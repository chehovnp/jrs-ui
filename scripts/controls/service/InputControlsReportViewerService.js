...
var fetchRequestParamsThroughURL = function fetchRequestParamsThroughURL(structure, allRequestParameters) {
  var fetchLabelOfControls = _.pluck(structure, 'id');

  var fetchKeysOfParams = _.keys(allRequestParameters);

  var requestedSelectionParams = {};
...
function () {
  ...
  _createClass(InputControlsReportViewerService, [{
    ...
    value: function fetchInputControlsInitialState(uri, allRequestParameters) {
      ...
      return this.inputControlsService.getInputControlsMetadata(uri).then(function (response) {
        if (response) {
          ...
          return _this.fetchInitialInputControlsValuesByUriAndStructure(uri, structure, allRequestParameters);
        }
    ...
    value: function fetchInitialInputControlsValuesByUri(uri, structure, preSelectedData) {
    ...
    value: function fetchInitialInputControlsValuesByUriAndStructure(uri, structure, preSelectionData) {
      var _this2 = this;

      var preSelectedData = fetchRequestParamsThroughURL(structure, preSelectionData);
      var initialPaginatedValuesOptions = structure.map(function (control) {
        return getControlPaginationOptionsByControlIdAndType(control.id, control.type, preSelectedData[control.id] ? {
      ...
      return $.when(initialPaginatedValuesPromise, selectedValuesPromise).then(function (paginatedValuesResponse, selectedValuesResponse) {
        ...
        var controlsMap = structure.reduce(function (memo, control) {
          return _objectSpread({}, memo, _defineProperty({}, control.id, preSelectedData[control.id] ? _objectSpread({}, control, {
            value: preSelectedData[control.id]
        ...
        var paginationOptionsPerControl = getPaginatedValuesOptionsForQueryControls(structure, controlsMap, function (option) {
          return preSelectedData[option.name] ? _objectSpread({}, option, {
            value: preSelectedData[option.name]
          }) : _objectSpread({}, option, {
            select: RestParamsEnum.SELECTED_VALUES
          });
        }, function (options) {
          _this2.inputControlsService.setCacheValueForControlPaginatedValues(uri, options, paginatedValues);
        });
      ...
      return this.inputControlsService.getInputControlsPaginatedValues(uri, paginationOptions).then(function (response) {
        paginationOptions.forEach(function (controlPaginationOptions) {
          ...
          if (paginationOptionsPerControl[controlId]) {
            _this3.inputControlsService.setCacheValueForControlPaginatedValues(uri, paginationOptionsPerControl[controlId], response);
          }
        });
        return response;
      });
